...
def test_valid_iou_calculation():
    ...
    try:
        ...
        tolerance = 0.01
        assert abs(iou - expected_iou) < tolerance, f"IOU should be ~{expected_iou:.3f}, got {iou:.3f}"
...
def test_self_intersecting_polygon_handling():
    ...
    try:
        ...
        assert iou >= 0.0 or iou == -1.0, f"Self-intersecting polygon should return valid IOU or -1.0, got {iou}"
        
        if iou >= 0.0:
            assert iou <= 1.0, f"Valid IOU should be <= 1.0, got {iou}"
            print(f"✅ Self-intersecting polygon successfully repaired (IOU = {iou:.3f})")
        else:
...
def test_error_logging():
    ...
    try:
        ...
        iou = runner._compute_iou(None, Polygon([(0, 0), (1, 0), (1, 1), (0, 1), (0, 0)]))
        assert iou == -1.0, "Should return -1.0 for None geometry"
        
        # Check that error was logged
        log_output = log_stream.getvalue()
        assert len(log_output) > 0, "Should have logged error message"
        assert "None" in log_output or "cannot compute" in log_output, "Should log specific error"
...
def test_multipolygon_handling():
    ...
    try:
        ...
        assert iou >= 0.0 or iou == -1.0, f"MultiPolygon should return valid IOU or -1.0, got {iou}"
...
def test_validation_helper_methods():
    ...
    try:
        ...
        assert runner._is_valid_area_geometry(polygon) == True, "Should accept valid polygon"
        assert runner._is_valid_area_geometry(point) == False, "Should reject point for area calc"
