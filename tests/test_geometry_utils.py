...
class TestSingleGeometryDiagnosis:
    ...
    def test_duplicate_points_diagnosis(self):
        ...
        if not diagnosis["is_valid"]:
            assert (
                "duplicate_points" in diagnosis["issues"]
                or "repeated_point" in diagnosis["explanation"].lower()
            )
    ...
    def test_geometry_analysis_error_handling(self):
        ...
        mock_geom = Mock()
        mock_geom.is_empty = False  # Not empty so it continues to bounds check
        ...
        assert (
            "bounds_error" in diagnosis["issues"]
            or "analysis_error" in diagnosis["issues"]
        )
        assert diagnosis["is_critical"] is True
...
class TestInvalidCoordinateHandling:
    ...
    def test_nan_coordinate_detection(self):
        ...
        with patch("scripts.geometry_utils.explain_validity") as mock_explain:
    ...
    def test_gdf_with_invalid_coordinates(self):
        ...
        # Create invalid geometry with self-intersection
        invalid_geom = Polygon([(0, 0), (0, 1), (1, 1), (1, 0), (0.5, 0.5), (0, 0)])

        gdf = gpd.GeoDataFrame({"id": [1, 2], "geometry": [valid_geom, invalid_geom]})

        # Use actual invalid geometry instead of mocking
        result_gdf = self.diag.diagnose_and_repair_geometries(gdf, "test data")

        # Should repair or handle the invalid geometry
        assert len(result_gdf) >= 1  # At least the valid one should remain
        # The invalid geometry should either be repaired or removed
        if len(result_gdf) == 2:
            # If both remain, the invalid one should be repaired
            assert all(geom.is_valid for geom in result_gdf.geometry)
...
class TestProgressiveRepairStrategies:
    ...
    def test_simplify_holes_strategy(self):
        ...
        repaired = self.diag._apply_repair_strategy(
            polygon_with_hole, "simplify_holes", 0
        )
...
class TestComprehensiveRepairWorkflow:
    ...
    def test_progressive_repair_application(self):
        ...
        coords = [
            (
                np.cos(a) + np.random.normal(0, 1e-10),
                np.sin(a) + np.random.normal(0, 1e-10),
            )
            for a in angles
        ]
        ...
        gdf = gpd.GeoDataFrame(
            {"id": [1, 2], "geometry": [self_intersecting, complex_polygon]}
        )

        result = self.diag.diagnose_and_repair_geometries(
            gdf, "progressive repair test"
        )
    ...
    def test_repair_statistics_tracking(self):
        ...
        gdf = gpd.GeoDataFrame({"id": [1], "geometry": [invalid_polygon]})
    ...
    def test_invalid_geometry_action_remove(self):
        ...
        # Create an invalid geometry (bowtie/figure-8 shape)
        unrepairable_geom = Polygon([(0, 0), (1, 1), (1, 0), (0, 1), (0, 0)])

        gdf = gpd.GeoDataFrame(
            {"id": [1, 2], "geometry": [valid_geom, unrepairable_geom]}
        )
    ...
    def test_invalid_geometry_action_convert_to_point(self):
        ...
        # Create invalid geometry (bowtie/figure-8 shape)
        ...
        # Should have both geometries, second one converted to point or repaired
        assert len(result) >= 1  # At least valid geometry should remain
        if len(result) == 2 and result.iloc[1].geometry.geom_type == "Point":
            # Successfully converted to point
            assert True
        else:
            # Either removed or successfully repaired
            assert True
...
class TestMemoryEfficientValidation:
    ...
    def test_memory_cleanup_during_processing(self):
        ...
        for i in range(100):
            ...
            angles = np.linspace(0, 2 * np.pi, num_points)
            coords = [
                (np.cos(a) * 100 + i * 200, np.sin(a) * 100 + i * 200) for a in angles
            ]
            coords.append(coords[0])
            complex_geometries.append(Polygon(coords))

        gdf = gpd.GeoDataFrame({"id": range(100), "geometry": complex_geometries})
    ...
    def test_chunked_processing_simulation(self):
        ...
        for chunk_start in range(0, total_geoms, chunk_size):
            ...
            chunk_gdf = gpd.GeoDataFrame(
                {"id": range(chunk_start, chunk_end), "geometry": chunk_geoms}
            )
...
class TestErrorPropagationAndFailureModes:
    ...
    def test_analysis_exception_handling(self):
        ...
        # Create an invalid geometry that will cause analysis errors
        invalid_geom = Polygon([(0, 0), (1, 1), (1, 0), (0, 1), (0, 0)])

        gdf = gpd.GeoDataFrame({"id": [1], "geometry": [invalid_geom]})

        # Mock the diagnosis method to raise an exception
        with patch.object(
            self.diag,
            ...
            # Should handle the error gracefully - either empty or original data
    ...
    def test_partial_failure_handling(self):
        ...
        valid_geom = Polygon([(0, 0), (1, 0), (1, 1), (0, 1), (0, 0)])
        invalid_geom1 = Polygon([(0, 0), (1, 1), (1, 0), (0, 1), (0, 0)])
        invalid_geom2 = Polygon([(0, 0), (1, 1), (1, 0), (0, 1), (0, 0)])

        gdf = gpd.GeoDataFrame(
            {"id": [1, 2, 3], "geometry": [valid_geom, invalid_geom1, invalid_geom2]}
        )
        ...
        with patch.object(self.diag, "_apply_repair_strategy") as mock_repair:
            ...
            mock_repair.side_effect = [
                make_valid(invalid_geom1),  # Success
                None,  # Failure
            ]

            result = self.diag.diagnose_and_repair_geometries(
                gdf, "partial failure test"
            )

            # Should have valid and repaired geometries, remove failed one
            assert len(result) <= 3  # At least some geometries should remain
    ...
    def test_logging_during_failures(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            ...
            # Mock repair to fail so we get warnings about remaining invalid geometries
            with patch.object(self.diag, "_apply_repair_strategy", return_value=None):
            ...
            assert any(
                ...
                or "geometry" in record.message.lower()
    ...
    def test_critical_error_identification(self):
        ...
        # The geometry is invalid, so it should be detected
...
class TestSpatialEdgeCases:
    ...
    def test_nested_holes_edge_case(self):
        ...
        try:
            ...
            if not diagnosis["is_valid"]:
                assert (
                    "nested" in " ".join(diagnosis["issues"]).lower()
                    or "hole" in diagnosis["explanation"].lower()
                )
        except Exception:
    ...
    def test_zero_area_polygon(self):
        ...
        if diagnosis["area"] == 0:
            assert (
                "extremely_small_area" in diagnosis["issues"]
                or not diagnosis["is_valid"]
            )
...
class TestProgressiveRepairIntegration:
    ...
    def test_repair_with_all_strategies_failing(self):
        ...
        # Mock the make_valid function in the geometry_utils module
        with patch(
            "scripts.geometry_utils.make_valid",
            side_effect=Exception("make_valid failed"),
        ):
            result = self.diag._apply_repair_strategy(invalid_geom, "make_valid", 0)

            # Should return None when repair fails
    ...
    def test_repair_statistics_accumulation(self):
        ...
        gdf = gpd.GeoDataFrame({"id": range(5), "geometry": geoms})
        ...
        total_attempted = sum(
            stats["attempted"] for stats in repair_stats["repair_counts"].values()
        )
        ...
        def diagnose_geometry(geom_index):
            try:
                ...
                coords = [
                    ...
                    (geom_index + 1, geom_index),
                    (geom_index + 1, geom_index + 1),
...
class TestRecommendationGeneration:
    ...
    def test_high_invalid_rate_recommendation(self):
        ...
        final_stats = {"total_invalid": 5}

        # Set up repair stats
        self.diag.repair_stats = {"repair_counts": {}}

        recommendations = self.diag._generate_geometry_recommendations(
            original_stats, final_stats
        )

        assert any(
            "high invalid geometry rate" in rec.lower() for rec in recommendations
        )
    ...
    def test_self_intersection_recommendation(self):
        ...
        original_stats = {
            "total_features": 100,
            "total_invalid": 10,
            "issue_types": {"self_intersection": 8},  # Many self-intersections
        }
    ...
    def test_duplicate_points_recommendation(self):
        ...
        original_stats = {
            "total_features": 100,
            "total_invalid": 15,
            "issue_types": {"duplicate_points": 12},  # Many duplicate points
        }
    ...
    def test_small_geometry_recommendation(self):
        ...
        original_stats = {
            "total_features": 50,
            "total_invalid": 5,
            "issue_types": {"extremely_small_area": 3},
        }
        final_stats = {"total_invalid": 2}
