...
class TestIncompatibleCoordinateSystemTransformations:
    ...
    def test_geographic_to_projected_boundary_errors(self):
        ...
        gdf = gpd.GeoDataFrame(
            {"id": range(len(invalid_coords)), "geometry": invalid_coords},
            crs="EPSG:4326",
        )
    ...
    def test_datum_incompatibility_detection(self):
        ...
        gdf_nad83 = gpd.GeoDataFrame(
            {"id": range(len(original_coords)), "geometry": original_coords},
            crs="EPSG:4269",
        )  # NAD83
        ...
        # Modern PROJ handles NAD83 to WGS84 transformations seamlessly
        # The key test is that transformation completes without errors
        assert len(gdf_wgs84) == len(
            gdf_nad83
        ), "Should preserve all features during transformation"
        assert gdf_wgs84.crs == "EPSG:4326", "Should be transformed to WGS84"

        # Verify all coordinates are valid (not NaN or infinite)
        for geom in gdf_wgs84.geometry:
            assert geom.is_valid, "Transformed geometry should be valid"
            assert not (
                geom.x != geom.x or geom.y != geom.y
            ), "Coordinates should not be NaN"  # NaN check
            assert (
                abs(geom.x) < 180 and abs(geom.y) < 90
            ), "Coordinates should be within valid geographic bounds"
    ...
    def test_projection_validity_outside_domain(self):
        ...
        gdf = gpd.GeoDataFrame(
            {"id": range(len(colorado_coords)), "geometry": colorado_coords},
            crs="EPSG:4326",
        )
        ...
        try:
            ...
            print(
                f"State Plane projection correctly failed for out-of-domain coordinates: {e}"
            )
    ...
    def test_transformation_reversibility(self):
        ...
        for orig, back in zip(gdf_original.geometry, gdf_back.geometry):
            ...
            assert (
                diff_x < tolerance and diff_y < tolerance
...
class TestDatumShiftErrors:
    ...
    def test_nad27_to_wgs84_shift_detection(self):
        ...
        gdf_nad27 = gpd.GeoDataFrame(
            {"id": range(len(self.test_coords)), "geometry": self.test_coords},
            crs="EPSG:4267",
        )  # NAD27
    ...
    def test_historical_datum_accuracy(self):
        ...
        for coord in problem_coords:
            ...
            try:
                ...
                assert (
                    diff_x > 1e-6 or diff_y > 1e-6
                ), "Historical datum transformation should change coordinates"
...
class TestHighLatitudeTransformationPrecision:
    ...
    def test_polar_projection_precision(self):
        ...
        gdf = gpd.GeoDataFrame(
            {"id": range(len(self.high_lat_coords)), "geometry": self.high_lat_coords},
            crs="EPSG:4326",
        )
            ...
            try:
                ...
                for orig, back in zip(gdf.geometry, back_transformed.geometry):
                    if back and not back.is_empty:
                        ...
                        if lat > 80:
                            tolerance = (
                                0.1  # Larger tolerance for extreme polar regions
                            )
                        elif lat > 70:
                        ...
                        if diff_x > tolerance or diff_y > tolerance:
                            print(
                                f"Precision loss in {description}: "
    ...
    def test_longitude_wraparound_issues(self):
        ...
        gdf = gpd.GeoDataFrame(
            {"id": range(len(wraparound_coords)), "geometry": wraparound_coords},
            crs="EPSG:4326",
        )
        ...
        try:
            ...
            for idx, (orig, back) in enumerate(
                zip(gdf.geometry, back_transformed.geometry)
            ):
                if back and not back.is_empty:
                    ...
                    if abs(orig.y) > 89:
                        ...
                        assert (
                            lat_diff < 0.1
                        ), f"Latitude precision loss too high: {lat_diff}"
                    else:
                        assert (
                            lon_diff < 1.0 and lat_diff < 0.01
    ...
    def test_meridian_convergence_effects(self):
        """Test transformation precision at high latitudes."""
        # Test coordinates at high latitudes where projection effects are more pronounced
        high_latitude_coords = [
            Point(-105.0, 30.0),  # Low latitude (baseline)
            Point(-105.0, 60.0),  # High latitude
            Point(-105.0, 75.0),  # Very high latitude
        ]

        gdf = gpd.GeoDataFrame(
            {"id": range(len(high_latitude_coords)), "geometry": high_latitude_coords},
            crs="EPSG:4326",
        )

        # Project to UTM (which handles high latitudes)
        projected = gdf.to_crs("EPSG:32613")  # UTM Zone 13N

        # The key test is that transformation works at high latitudes
        # without producing invalid coordinates
        for geom in projected.geometry:
            assert geom.is_valid, "Projected geometry should be valid"
            assert not (
                geom.x != geom.x or geom.y != geom.y
            ), "Coordinates should not be NaN"
            # UTM coordinates should be reasonable (not extreme values)
            assert (
                abs(geom.x) < 1e7 and abs(geom.y) < 1e7
            ), "UTM coordinates should be reasonable"

        # Verify Y coordinates increase with latitude (basic sanity check)
        y_coords = [geom.y for geom in projected.geometry]
        assert (
            y_coords[1] > y_coords[0]
        ), "Higher latitude should have higher UTM Y coordinate"
        assert (
            y_coords[2] > y_coords[1]
        ), "Even higher latitude should have even higher UTM Y coordinate"
...
class TestCoordinateValidationFailures:
    ...
    def setup_method(self):
        ...
        self.validation_config = {
            "coordinate_validation": {
                ...
                "projected_bounds": {"max_abs_coord": 1e8},  # 100 million meters
            }
        }
    ...
    def test_coordinate_range_validation_bypass(self):
        ...
        gdf = gpd.GeoDataFrame(
            {"id": range(len(problematic_coords)), "geometry": problematic_coords},
            crs="EPSG:4326",
        )
    ...
    def test_coordinate_corruption_detection(self):
        ...
        with patch.object(gdf, "to_crs") as mock_to_crs:
            ...
            corrupted_gdf = gpd.GeoDataFrame(
                {"id": range(len(corrupted_coords)), "geometry": corrupted_coords},
                crs="EPSG:5070",
            )
...
class TestCoordinateOverflowUnderflowScenarios:
    ...
    def test_extreme_coordinate_values(self):
        ...
        gdf = gpd.GeoDataFrame(
            {"id": range(len(extreme_coords)), "geometry": extreme_coords},
            crs="EPSG:4326",
        )
                ...
                for idx, geom in enumerate(projected.geometry):
                    if geom and not geom.is_empty:
                        ...
                        if (
                            ...
                            or abs(x) > 1e15
                            or abs(y) > 1e15
                            or (abs(x) < 1e-15 and x != 0)
                            or (abs(y) < 1e-15 and y != 0)
                        ):
    ...
    def test_web_mercator_polar_overflow(self):
        ...
        gdf = gpd.GeoDataFrame(
            {"id": range(len(near_polar_coords)), "geometry": near_polar_coords},
            crs="EPSG:4326",
        )
                ...
                if back and not back.is_empty:
                    ...
                    if abs(orig.y) > 85:
                        ...
                        if lat_diff > 0.01:  # More than ~1km error
                            print(
                                f"High precision loss at latitude {orig.y}: {lat_diff}°"
                            )
        ...
        for idx, geom in enumerate(projected.geometry[:2]):  # First two points
            if geom and not geom.is_empty:
                ...
                if diff_x == 0 and diff_y == 0:
                    print(
                        f"Precision limit reached: point {idx} identical to reference after projection"
                    )
                else:
                    print(
                        f"Precision preserved: point {idx} differs by ({diff_x}, {diff_y}) meters"
                    )
...
class TestCRSTransformationIntegration:
            ...
            try:
                ...
                for idx, geom in enumerate(transformed.geometry):
                    if geom and not geom.is_empty:
                        ...
                        if any(abs(coord) > 5000000 for coord in bounds) or any(
                            math.isnan(coord) or math.isinf(coord) for coord in bounds
                        ):
                            invalid_transformations.append(idx)

                if invalid_transformations:
                    print(
                        f"Invalid transformations detected: {invalid_transformations}"
                    )
    ...
    def test_truth_extractor_coordinate_consistency(self):
        ...
        geometries = [
            Point(lon, lat)
            for lon, lat in zip(
                pour_points["Pour_Point_Lon"], pour_points["Pour_Point_Lat"]
            )
        ]
    ...
    def test_benchmark_runner_crs_validation(self):
        ...
        runner = BenchmarkRunner(
            sample_df=pd.DataFrame(), truth_path="dummy", config=config
        )
