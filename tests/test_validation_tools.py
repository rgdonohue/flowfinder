...
def test_validate_intermediate_csv_bad(bad_csv):
    ...
    # The bad CSV is readable but missing the required 'id' column
    assert (
        "Missing columns: ['id']" in result["issues"][0]
        or "Error reading CSV" in result["issues"][0]
    )
...
def test_detect_performance_regression(tmp_path, temp_json):
    ...
    result = vt.detect_performance_regression(
        current, str(baseline_path), {"iou": 0.05, "runtime": 0.1}
    )
