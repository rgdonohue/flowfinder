...
def test_configuration_inheritance():
    ...
    print(
        f"   Environment timeout: {config['benchmark']['timeout_seconds']}s (overridden by development)"
    )
    print(
        f"   Tool-specific threshold (flat): {config['benchmark']['success_thresholds']['flat']}"
    )
    ...
    local_overrides = {
        "benchmark": {"timeout_seconds": 15},
        "tool": {"additional_args": ["--verbose", "--debug"]},
    }

    config_with_overrides = manager.get_tool_config("flowfinder", local_overrides)
    print(
        f"   Final timeout: {config_with_overrides['benchmark']['timeout_seconds']}s (local override)"
    )
...
def test_tool_adapters():
    ...
    for tool_name in tools:
        ...
        try:
            ...
            print(
                f"   ✓ Environment variables: {len(adapter.get_environment_variables())} vars"
            )
...
def test_complete_workflow():
    ...
    print(
        f"   Success threshold (flat): {dev_config['benchmark']['success_thresholds']['flat']}"
    )
