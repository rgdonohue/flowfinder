...
def test_result_format_structure():
    ...
    try:
        ...
        print(
            f"✅ WatershedGeometry: {geometry.area_km2} km², centroid: ({geometry.centroid_lat}, {geometry.centroid_lon})"
        )
        ...
        performance = PerformanceMetrics(
            ...
            processing_stages={
                "flow_direction": 8.2,
                "flow_accumulation": 12.1,
                "watershed_extraction": 5.2,
            },
        )
        print(
            f"✅ PerformanceMetrics: {performance.runtime_seconds}s, efficiency: {performance.efficiency_score:.2f}"
        )

        # Test quality metrics
        quality = QualityMetrics.calculate_from_geometry(geometry)
        print(
            f"✅ QualityMetrics: compactness: {quality.compactness_ratio:.3f}, complexity: {quality.shape_complexity:.1f}"
        )
        ...
        print(
            f"✅ ToolSpecificData: {tool_data.tool_name.value}, algorithm: {tool_data.algorithm_used}"
        )
        ...
        print(
            f"✅ StandardizedWatershedResult: {result.result_id}, status: {result.status.value}"
        )
...
def test_multi_tool_comparison():
    ...
    try:
        ...
        comparison = create_multi_tool_comparison(
            tool_results=tool_results, pour_point=(40.0, -105.5), environment="testing"
        )
