...
def test_full_pipeline_validation(tmp_path):
    ...
    result = vt.validate_intermediate_csv(
        str(csv_path), ["id", "area_km2", "pour_point_x"]
    )
    ...
    current_metrics = {
        "iou": 0.89,
        "runtime": 32.0,
    }  # Larger differences to trigger regression
    thresholds = {"iou": 0.05, "runtime": 0.2}
    result = vt.detect_performance_regression(
        current_metrics, str(baseline_path), thresholds
    )
