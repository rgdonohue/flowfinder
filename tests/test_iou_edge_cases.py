...
class TestIOUDegenerateGeometries:
    ...
    def setup_method(self):
        ...
        self.runner = BenchmarkRunner(sample_df=pd.DataFrame(), truth_path="dummy", config=self.config)
...
class TestIOUExtremelySmallOverlaps:
    ...
    def test_iou_tiny_overlap_precision_limits(self):
        ...
        poly2 = Polygon([(5e-7, 5e-7), (1.5e-6, 5e-7), (1.5e-6, 1.5e-6), (5e-7, 1.5e-6), (5e-7, 5e-7)])
    ...
    def test_iou_microscopic_polygons(self):
        ...
        tiny2 = Polygon([(5e-8, 5e-8), (1.5e-7, 5e-8), (1.5e-7, 1.5e-7), (5e-8, 1.5e-7), (5e-8, 5e-8)])
    ...
    def test_iou_different_coordinate_scales(self):
        ...
        tiny = Polygon([(500, 500), (500 + 1e-6, 500), (500 + 1e-6, 500 + 1e-6), (500, 500 + 1e-6), (500, 500)])
...
class TestIOUSelfIntersectingPolygons:
    ...
    def test_iou_one_valid_one_invalid(self):
        ...
        invalid_poly = Polygon([(1, 1), (3, 3), (3, 1), (1, 3), (1, 1)])  # Self-intersecting
    ...
    def test_iou_repair_validation(self):
        ...
        # Create polygon that might fail repair
        problematic = Polygon([(0, 0), (1, 1), (0, 1), (1, 0), (0.5, 0.5), (0, 0)])
        valid_poly = Polygon([(0, 0), (1, 0), (1, 1), (0, 1), (0, 0)])
        
        # Mock make_valid to return an invalid result
        with patch('shapely.validation.make_valid') as mock_make_valid:
            # Return a geometry that's still invalid after "repair"
            mock_invalid = Mock()
            mock_invalid.is_valid = False
            mock_invalid.is_empty = False
            mock_invalid.geom_type = 'Polygon'
            mock_make_valid.return_value = mock_invalid
            
            iou = self.runner._compute_iou(problematic, valid_poly)
            
            # Should detect failed repair and return -1.0
            assert iou == -1.0, "Should detect failed geometry repair"
...
class TestIOUInvalidReturnValueHandling:
    ...
    def test_intersection_operation_failure(self):
        ...
        # Mock intersection to raise exception
        with patch.object(poly1, 'intersection', side_effect=Exception("Intersection failed")):
            iou = self.runner._compute_iou(poly1, poly2)
    ...
    def test_union_operation_failure(self):
        ...
        # Mock unary_union to raise exception
        with patch('shapely.ops.unary_union', side_effect=Exception("Union failed")):
            iou = self.runner._compute_iou(poly1, poly2)
    ...
    def test_area_calculation_failure(self):
        ...
        # Mock to make intersection.area raise exception
        with patch.object(self.runner, '_compute_iou', wraps=self.runner._compute_iou) as mock_iou:
            # Create mock intersection that fails on area access
            ...
            type(mock_intersection).area = PropertyMock(side_effect=Exception("Area calculation failed"))
            
            with patch('shapely.validation.make_valid', return_value=poly1):
                with patch.object(poly1, 'intersection', return_value=mock_intersection):
                    iou = self.runner._compute_iou(poly1, poly2)
...
class TestIOUPrecisionLossScenarios:
    ...
    def test_iou_accumulated_precision_errors(self):
        ...
        for i in range(100):
            ...
            offset = 1e-14 * i
            transformed_coords = [(x + offset, y + offset) for x, y in transformed_coords]
...
class TestIOUMultiPolygonScenarios:
    ...
    def test_iou_multipolygon_vs_polygon(self):
        ...
        single_poly = Polygon([(0.5, 0.5), (1.5, 0.5), (1.5, 1.5), (0.5, 1.5), (0.5, 0.5)])
    ...
    def test_iou_multipolygon_with_holes(self):
        ...
        test_poly = Polygon([(0.5, 0.5), (1.5, 0.5), (1.5, 1.5), (0.5, 1.5), (0.5, 0.5)])
...
class TestIOULoggingAndErrorReporting:
    ...
    def test_iou_debug_logging_for_valid_calculation(self):
        ...
        assert iou == pytest.approx(0.25, abs=1e-6), "IOU should be 0.25 for this overlap"
...
class TestIOUIntegrationWithCalculateMetrics:
    ...
    def test_calculate_metrics_with_invalid_iou(self):
        ...
        with patch.object(self.runner, '_validate_crs_transformation') as mock_transform:
            mock_gdf = Mock()
            mock_gdf.iloc = [Mock()]
