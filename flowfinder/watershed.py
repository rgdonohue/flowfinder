...
class WatershedExtractor:
    ...
    def __init__(self, flow_accumulation_calc, stream_threshold: int = 1000):
        ...
        self.logger.info(f"Watershed extractor initialized with stream threshold: {stream_threshold}")
    ...
    def extract_watershed(self, row: int, col: int) -> np.ndarray:
        ...
        try:
            ...
            if not watershed_cells:
                raise WatershedError(f"No watershed cells found for pour point ({row}, {col})")
    ...
    def extract_watershed_with_streams(self, row: int, col: int) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _extract_stream_network(self, watershed_cells: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(watershed_cells[0])):
            ...
            if (0 <= row < flow_accum.shape[0] and 
                0 <= col < flow_accum.shape[1] and
                flow_accum[row, col] >= self.stream_threshold):
    ...
    def extract_watershed_polygon(self, row: int, col: int) -> Optional[List[Tuple[float, float]]]:
        """
    ...
    def _cells_to_polygon_vertices(self, boundary_cells: List[Tuple[int, int]]) -> List[Tuple[float, float]]:
        """
        Convert boundary cells to polygon vertices.
    ...
    def _sort_vertices_for_polygon(self, vertices: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
        """
        Sort vertices to form a proper polygon.
