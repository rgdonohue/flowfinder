...
class DInfinityFlowDirection:
    ...
    def calculate_dinf_flow_direction(self, dem_array: np.ndarray, 
                                    nodata_value: float) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _calculate_dinf_for_cell(self, dem_array: np.ndarray, 
                               row: int, col: int, nodata_value: float) -> Tuple[float, float]:
        """
        ...
        for i, facet in enumerate(facets):
            ...
            if (dem_array[r1, c1] == nodata_value or 
                dem_array[r2, c2] == nodata_value):
                continue
    ...
    def _analyze_triangular_facet(self, center_elev: float, 
                                 elev1: float, elev2: float, 
                                 facet_index: int) -> Tuple[float, float]:
        """
        ...
        base_angles = [0, np.pi/4, np.pi/2, 3*np.pi/4, 
...
class StreamBurning:
    ...
    def burn_streams(self, dem_array: np.ndarray, 
        ...
        try:
            ...
            for line_idx, stream_line in enumerate(stream_lines):
                ...
                self.logger.debug(f"Burned stream line {line_idx + 1}: {pixels_burned} pixels")
    ...
    def _burn_stream_line(self, dem_array: np.ndarray, 
    ...
    def _smooth_burned_areas(self, dem_array: np.ndarray, 
...
class HydrologicEnforcement:
    ...
    def enforce_drainage_direction(self, flow_direction: np.ndarray,
                                 dem_array: np.ndarray,
                                 stream_network: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def _is_flow_direction_valid(self, dem_array: np.ndarray,
        ...
        # Check bounds
        if (downstream_row < 0 or downstream_row >= dem_array.shape[0] or
            downstream_col < 0 or downstream_col >= dem_array.shape[1]):
            return False
    ...
    def _calculate_corrected_flow_direction(self, dem_array: np.ndarray,
                                          row: int, col: int) -> int:
        """
