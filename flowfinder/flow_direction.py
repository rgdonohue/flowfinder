...
class FlowDirectionCalculator:
    ...
    def __init__(
        self,
        dem_data: rasterio.DatasetReader,
        method: str = "d8",
        ...
        self.logger.info(
            f"Flow direction calculator initialized with method: {self.method}"
        )
    ...
    def calculate(self) -> np.ndarray:
        ...
        try:
            ...
            if self.method == "d8":
                ...
                self.flow_direction, self.flow_slopes = (
                    self.dinf_calculator.calculate_dinf_flow_direction(
                        dem_array, self.dem_data.nodata
                    )
                )
            ...
            if self.method == "d8":
                self.flow_direction = (
                    self.hydrologic_enforcer.enforce_drainage_direction(
        ...
        while changed and iterations < max_iterations:
            ...
            for i in range(1, height - 1):
                for j in range(1, width - 1):
                    ...
                    valid_neighbors = [
                        n for n in neighbors if n != self.dem_data.nodata
                    ]
            ...
            for j in range(1, width - 1):
                ...
                for k, (di, dj) in enumerate(offsets):
                    ...
                    if (
                        0 <= ni < height
                        and 0 <= nj < width
                        and dem_array[ni, nj] != self.dem_data.nodata
                    ):
    ...
    def get_downstream_cell(self, row: int, col: int) -> Optional[Tuple[int, int]]:
        ...
        if not (
            0 <= row < self.flow_direction.shape[0]
            and 0 <= col < self.flow_direction.shape[1]
        ):
            return None
