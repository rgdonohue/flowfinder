...
class PerformanceMonitor:
    ...
    def finish_monitoring(self, dem_size_pixels: int, 
                         watershed_area_km2: float) -> PerformanceMetrics:
        """
...
class TopologyValidator:
    ...
    def validate_watershed_topology(self, watershed_polygon: Polygon,
                                  pour_point: Tuple[float, float],
                                  dem_bounds: Tuple[float, float, float, float]) -> TopologyMetrics:
        """
        ...
        try:
            ...
            if not is_valid:
                errors.append(f"Invalid geometry: {explain_validity(watershed_polygon)}")
            ...
            if perimeter_km > 0 and area_km2 > 0:
                ...
                if shape_ratio > 100:  # Very elongated or fragmented
                    errors.append(f"Watershed shape suspicious (high perimeter/area ratio: {shape_ratio:.1f})")
            ...
            if errors:
                ...
                self.logger.info(f"Topology validation passed (quality score: {quality_score:.2f})")
    ...
    def _validate_watershed_bounds(self, polygon: Polygon, 
                                 dem_bounds: Tuple[float, float, float, float]) -> bool:
        ...
        try:
            ...
            return (poly_bounds[0] >= minx - tolerance and
                   poly_bounds[1] >= miny - tolerance and
                   poly_bounds[2] <= maxx + tolerance and
                   poly_bounds[3] <= maxy + tolerance)
        except:
            return True  # If bounds check fails, assume valid
    
    def _calculate_topology_quality_score(self, is_valid: bool, is_simple: bool,
                                        holes_count: int, self_intersections: int,
    ...
    def repair_watershed_topology(self, watershed_polygon: Polygon) -> Polygon:
        ...
        try:
            ...
            try:
                ...
                if buffered.is_valid and hasattr(buffered, 'area') and buffered.area > 0:
            ...
            try:
                ...
                if hull.is_valid and hasattr(hull, 'area') and hull.area > 0:
                    self.logger.warning("Used convex hull as topology repair (may be inaccurate)")
                    return hull
            except:
                pass
...
class AccuracyAssessment:
    ...
    def calculate_iou_score(self, predicted_polygon: Polygon, 
                           truth_polygon: Polygon) -> float:
        """
    ...
    def assess_watershed_quality(self, watershed_polygon: Polygon,
                               performance_metrics: PerformanceMetrics,
                               topology_metrics: TopologyMetrics,
                               reference_polygon: Optional[Polygon] = None) -> Dict[str, Any]:
        """
        ...
        if reference_polygon is not None:
            try:
                iou_score = self.calculate_iou_score(watershed_polygon, reference_polygon)
                assessment['accuracy'] = {
                    'iou_score': iou_score,
    ...
    def _calculate_overall_quality(self, assessment: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if 'accuracy' in assessment:
                accuracy_ok = assessment['accuracy'].get('meets_95_percent_target', False)
