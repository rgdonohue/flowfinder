...
from .exceptions import DEMError, WatershedError, ValidationError
    ...
    try:
        ...
        if polygon.is_empty:
            ...
            geojson = {
                ...
                "properties": {"source": "FLOWFINDER", "version": "1.0.0"},
            }
...
def save_shapefile(polygon, output_path: str) -> None:
    ...
    try:
        ...
        # shapely.geometry.Polygon imported but not used
    ...
    try:
        ...
        with FlowFinder(args.dem, config) as flowfinder:
            ...
            watershed, quality_metrics = flowfinder.delineate_watershed(
                lat=args.lat, lon=args.lon, timeout=args.timeout
            )
...
def validate_command(args) -> None:
    ...
    try:
        ...
        with FlowFinder(args.dem) as flowfinder:
            ...
            print(
                f"  Size: {flowfinder.dem_data.width}x{flowfinder.dem_data.height} pixels"
            )
...
def info_command(args) -> None:
    ...
    try:
        ...
        with rasterio.open(args.dem) as dataset:
            ...
            if len(valid_data) > 0:
                print(
                    f"  Elevation range: {valid_data.min():.1f}m - {valid_data.max():.1f}m"
                )
...
Examples:
  ...
  flowfinder delineate --dem dem.tif --lat 40.0 --lon -105.0 --config config.yaml
    ...
    delineate_parser = subparsers.add_parser(
        "delineate", help="Delineate watershed for a pour point"
    )
    delineate_parser.add_argument("--dem", required=True, help="Path to DEM file")
    delineate_parser.add_argument(
        "--lat", type=float, required=True, help="Latitude of pour point"
    )
    delineate_parser.add_argument(
        "--lon", type=float, required=True, help="Longitude of pour point"
    )
    delineate_parser.add_argument(
        "--output", help="Output file path (.geojson or .shp)"
    )
    ...
    delineate_parser.add_argument(
        "--method", choices=["d8", "dinf", "mfd"], help="Flow direction method"
    )
    delineate_parser.add_argument(
    ...
    # Validate command
    validate_parser = subparsers.add_parser("validate", help="Validate DEM file")
    ...
    # Info command
    info_parser = subparsers.add_parser("info", help="Display DEM information")
