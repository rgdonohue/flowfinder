...
class OptimizedDepressionFilling:
        ...
        try:
            ...
            for i in range(height):
                for j in range(width):
                    if valid_mask[i, j] and self._is_border_cell(
                        i, j, height, width, valid_mask
                    ):
            ...
            while pq:
                ...
                for dr, dc in neighbors:
                    ...
                    if (
                        0 <= new_row < height
                        and 0 <= new_col < width
                        and valid_mask[new_row, new_col]
    ...
    def _is_border_cell(
        self, row: int, col: int, height: int, width: int, valid_mask: np.ndarray
    ) -> bool:
        """
        ...
        for dr, dc in neighbors:
            ...
            if (
                0 <= new_row < height
                and 0 <= new_col < width
                and not valid_mask[new_row, new_col]
            ):
                return True
...
class OptimizedFlowAccumulation:
    ...
    def calculate_flow_accumulation(
        self, flow_direction: np.ndarray, nodata_value: int = 0
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            self.logger.info(f"Flow accumulation completed in {runtime:.2f}s")
    ...
    def _build_flow_network(
        self, flow_direction: np.ndarray, valid_mask: np.ndarray
        ...
        for i in range(height):
            for j in range(width):
                ...
                if flow_code in d8_offsets:
                    ...
                    # Check bounds
                    if (
                        0 <= downstream_i < height
                        and 0 <= downstream_j < width
                        and valid_mask[downstream_i, downstream_j]
                    ):
    ...
    def _topological_accumulation(
        ...
        while queue:
            ...
            if (current_i, current_j) in downstream_map:
                for downstream_i, downstream_j in downstream_map[
                    (current_i, current_j)
                ]:
                    # Add current cell's accumulation to downstream cell
                    flow_accum[downstream_i, downstream_j] += flow_accum[
                        current_i, current_j
                    ]
...
class OptimizedPolygonCreation:
    ...
    def pixels_to_polygon(
        ...
        try:
            ...
            if simplify_tolerance > 0 and len(boundary_coords) > 3:
                boundary_coords = self._simplify_polygon(
                    boundary_coords, simplify_tolerance
                )
    ...
    def _clean_binary_mask(self, binary_mask: np.ndarray) -> np.ndarray:
        ...
        # Smooth boundaries
        smoothed_mask = morphology.binary_closing(cleaned_mask, morphology.disk(1))

        return smoothed_mask

    def _extract_boundary_coordinates(
        self, binary_mask: np.ndarray, row_offset: int, col_offset: int, transform: Any
    ) -> List[Tuple[float, float]]:
        """
    ...
    def _create_bounding_box_polygon(
        self, watershed_pixels: np.ndarray, transform: Any
    ) -> List[Tuple[float, float]]:
        """
