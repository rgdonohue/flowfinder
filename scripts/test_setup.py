...
class TestSetup:
    ...
    def create_sample_basin_data(self):
        ...
        sample_basins = pd.DataFrame({
            ...
            'huc12': ['130101010101', '130101010102', '130101010103', '130101010104', '130101010105'],
            'name': ['Sample Basin 1', 'Sample Basin 2', 'Sample Basin 3', 'Sample Basin 4', 'Sample Basin 5'],
            ...
            'elevation_zone': ['montane', 'montane', 'foothills', 'montane', 'foothills'],
            'state': ['CO', 'CO', 'CO', 'CO', 'CO']
        })
        
        basin_path = Path(self.config['storage']['processed_data_dir']) / "basin_sample.csv"
    ...
    def create_sample_truth_data(self):
        ...
        # Save CSV
        truth_csv_path = Path(self.config['storage']['processed_data_dir']) / "truth_polygons.csv"
        ...
        try:
            ...
            # Save GeoPackage
            truth_gpkg_path = Path(self.config['storage']['processed_data_dir']) / "truth_polygons.gpkg"
    ...
    def create_sample_benchmark_results(self):
        ...
        results_path = Path(self.config['storage']['processed_data_dir']) / "benchmark_results.json"
    ...
    def create_test_config(self):
        ...
        config_path = Path(self.config['storage']['processed_data_dir']) / "test_pipeline_config.yaml"
    ...
    def setup_test_environment(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"   python run_benchmark.py --config {self.config['storage']['processed_data_dir']}/test_pipeline_config.yaml --output test_results")
                logger.info("")
                logger.info("2. Test validation tools:")
                logger.info(f"   python scripts/validation_tools.py --validate-csv {self.config['storage']['processed_data_dir']}/basin_sample.csv --columns id,area_km2")
                logger.info("")
                logger.info("3. Test individual scripts:")
                logger.info(f"   python scripts/benchmark_runner.py --input {self.config['storage']['processed_data_dir']}/basin_sample.csv --truth {self.config['storage']['processed_data_dir']}/truth_polygons.csv --output test_benchmark")
...
Examples:
  ...
  python test_setup.py --validate
    ...
    parser.add_argument('--config', '-c', default='config/data_sources_test.yaml',
