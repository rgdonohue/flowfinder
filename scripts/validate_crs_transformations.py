...
def test_basic_crs_transformation():
    ...
    try:
        ...
        if (-2000000 <= x <= 2000000 and 0 <= y <= 3000000):
            ...
            print(f"❌ Transformation produced suspicious coordinates: ({x:.0f}, {y:.0f})")
            return False
...
def test_transformation_reversibility():
    ...
    try:
        ...
        if max_error < tolerance:
            print(f"✅ Transformation reversibility preserved (max error: {max_error:.2e})")
            return True
        else:
...
def test_datum_shift_detection():
    ...
    try:
        ...
        gdf_nad27 = gpd.GeoDataFrame({'geometry': [test_point]}, crs='EPSG:4267')  # NAD27
        gdf_wgs84 = gpd.GeoDataFrame({'geometry': [test_point]}, crs='EPSG:4326')  # WGS84
        ...
        if 10 < diff_meters < 500:
            ...
            print(f"❌ Excessive datum shift: {diff_meters:.1f} meters - possible error")
            return False
    ...
    try:
        ...
        try:
            ...
            if max_lat_error < 0.1:  # Less than ~10km error
                print(f"✅ High latitude precision acceptable (lat error: {max_lat_error:.6f}°)")
                return True
            else:
                print(f"❌ Excessive high latitude precision loss (lat error: {max_lat_error:.6f}°)")
                return False
            ...
            for idx, geom in enumerate(projected.geometry):
                if geom and not geom.is_empty:
                    ...
                    if (math.isnan(x) or math.isnan(y) or 
                        ...
                        abs(x) > 5000000 or abs(y) > 5000000):
                        invalid_coords.append(idx)
            
            if len(invalid_coords) > 0:
                print(f"✅ Detected {len(invalid_coords)} invalid projections (expected)")
                return True
            else:
                # All coordinates projected successfully - check if they're reasonable
                bounds = projected.total_bounds
                if (-3000000 <= bounds[0] <= 3000000 and 
                    -2000000 <= bounds[1] <= 3000000):
                    print("✅ All coordinates within reasonable US projection bounds")
                    return True
                else:
                    print(f"❌ Projected coordinates outside reasonable bounds: {bounds}")
                    return False
