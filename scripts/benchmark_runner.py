...
class BenchmarkRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            sample_df: Legacy parameter for backward compatibility
            truth_path: Legacy parameter for backward compatibility
            config: Legacy parameter for backward compatibility
        ...
        if config is not None:
            # Legacy mode: use provided config directly
            self.config = config
            self.config_manager = None
        else:
            # New mode: use hierarchical configuration manager
            ...
            self.config = self.config_manager.get_tool_config(
                primary_tool, local_overrides
            )

        # Add legacy configuration keys for backward compatibility
        self._add_legacy_config_keys()

        # Handle legacy data parameters
        ...
        self.logger.info(
            f"BenchmarkRunner initialized for environment '{environment}' with tools: {self.tools}"
        )

    def _get_tool_config(
        self, tool_name: str, local_overrides: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _add_legacy_config_keys(self) -> None:
        """Add legacy configuration keys for backward compatibility with old tests."""
        # Map new config structure to legacy keys that tests expect
        if (
            "coordinate_systems" in self.config
            and "processing_crs" in self.config["coordinate_systems"]
        ):
            self.config["projection_crs"] = self.config["coordinate_systems"][
                "processing_crs"
            ]
        elif "projection_crs" not in self.config:
            self.config["projection_crs"] = "EPSG:5070"  # Default

        if "benchmark" in self.config:
            # Map benchmark config to top-level keys
            if "timeout_seconds" in self.config["benchmark"]:
                self.config["timeout_seconds"] = self.config["benchmark"][
                    "timeout_seconds"
                ]
            if "success_thresholds" in self.config["benchmark"]:
                self.config["success_thresholds"] = self.config["benchmark"][
                    "success_thresholds"
                ]
            if "output_formats" in self.config["benchmark"]:
                self.config["output_formats"] = self.config["benchmark"][
                    "output_formats"
                ]
        ...
        if "success_thresholds" not in self.config:
            self.config["success_thresholds"] = {
                "default": 0.9,
                "flat": 0.95,
                "steep": 0.85,
            }
        if "centroid_thresholds" not in self.config:
            self.config["centroid_thresholds"] = {
                "default": 500,
                "flat": 200,
                "steep": 1000,
            }
        if "output_formats" not in self.config:
        ...
        if "performance_analysis" not in self.config:
            self.config["performance_analysis"] = {
                "runtime_target": 30,
                "iou_target": 0.90,
            }
    ...
    def _validate_config(self) -> None:
        ...
        # Validate success thresholds (legacy compatibility)
        success_thresholds = self.config.get("success_thresholds", {})
        ...
        # Validate centroid thresholds (legacy compatibility)
        centroid_thresholds = self.config.get("centroid_thresholds", {})
        for terrain, threshold in centroid_thresholds.items():
            if threshold <= 0:
                raise ValueError(f"Centroid threshold for {terrain} must be positive")

        # Validate timeout (legacy compatibility)
        timeout = self.config.get("timeout_seconds", 120)
        if timeout <= 0:
            raise ValueError("timeout_seconds must be positive")

        # Validate tool configuration (more lenient for testing)
        tool_config = self.config.get("tool", {})
        if not tool_config.get("executable"):
            self.logger.warning(
                "Tool executable not specified - may be running in test mode"
            )

        # Validate all tool adapters (skip if no config manager for legacy mode)
        if hasattr(self, "config_manager") and self.config_manager:
            validation_results = self.config_manager.validate_all_tools()
            failed_tools = [
                tool for tool, valid in validation_results.items() if not valid
            ]
            if failed_tools:
                self.logger.warning(
                    f"Configuration validation failed for tools: {failed_tools}"
                )
    ...
    def load_datasets(self, sample_path: str, truth_path: str) -> None:
        ...
        try:
            ...
            missing_cols = [
                col for col in required_cols if col not in self.sample_df.columns
            ]
            if missing_cols:
                raise ValueError(
                    f"Missing required columns in basin sample: {missing_cols}"
                )
            ...
            if (
                "Size_Class" not in self.sample_df.columns
                and "Area_km2" in self.sample_df.columns
            ):
                self.sample_df["Size_Class"] = self.sample_df["Area_km2"].apply(
                    self._get_size_class
                )

            self.logger.info(
                f"Loaded {len(self.sample_df)} sample basins and {len(self.truth_gdf)} truth polygons"
            )
    ...
    def run_benchmark(self) -> Dict[str, Any]:
        ...
        for i, (_, row) in enumerate(
            ...
            try:
                ...
                performance = self._assess_performance(
                    metrics["iou"], metrics["centroid_offset"], terrain_class
                )
                ...
                result = {
                    "ID": basin_id,
                    "IOU": (
                        round(metrics["iou"], 4) if metrics["iou"] is not None else None
                    ),
                    "Boundary_Ratio": (
                        round(metrics["boundary_ratio"], 4)
                        if metrics["boundary_ratio"] is not None
                        else None
                    ),
                    "Centroid_Offset_m": (
                        round(metrics["centroid_offset"], 1)
                        if metrics["centroid_offset"] is not None
                        else None
                    ),
                ...
                if metrics["status"] == "invalid_geometry":
                    ...
                    iou_str = (
                        f"{metrics['iou']:.3f}" if metrics["iou"] is not None else "N/A"
                    )

                self.logger.info(
                    f"Basin {basin_id} [{i+1}/{len(self.sample_df)}]: {status} | IOU={iou_str} | t={runtime:.1f}s"
                )
        ...
        self.logger.info(
            f"Benchmark completed: {len(self.results)} successful, {len(self.errors)} failed"
        )
        return benchmark_summary

    def _run_delineation(
        ...
        try:
            ...
            geojson_bytes = subprocess.check_output(
                cmd, timeout=timeout, stderr=subprocess.PIPE, env=env
            )
            ...
            return None, None, f"FLOWFINDER timeout (>" + str(timeout) + "s)"
            ...
            self.logger.warning(
                f"FLOWFINDER command not found, generating mock result for testing"
            )
            runtime = time.perf_counter() - start
    ...
    def _calculate_metrics(self, pred_poly: Any, truth_poly: Any) -> Dict[str, float]:
        ...
        proj_crs = self.config.get("coordinate_systems", {}).get(
        ...
        try:
            ...
            truth_gdf = gpd.GeoDataFrame(
                geometry=[truth_poly],
                crs=truth_poly.crs if hasattr(truth_poly, "crs") else None,
            )

            pred_proj_gdf = self._validate_crs_transformation(
                pred_gdf, "predicted polygon for metrics", proj_crs
            )
            if truth_gdf.crs is None:
                truth_gdf.crs = "EPSG:4326"  # Assume WGS84 if CRS missing
            truth_proj_gdf = self._validate_crs_transformation(
                truth_gdf, "truth polygon for metrics", proj_crs
            )
        ...
        try:
            ...
            try:
                ...
                if intersection is None or intersection.is_empty:
                    self.logger.debug(
                        "No intersection between geometries - returning 0.0"
                    )
                    return 0.0

                # Validate intersection result - handle special case of LineString intersection
                if intersection.geom_type in ("LineString", "MultiLineString", "Point"):
                    # Edge/point intersection means zero area overlap
                    self.logger.debug(
                        f"Intersection is {intersection.geom_type} - zero area overlap"
                    )
                    return 0.0
                elif not self._is_valid_area_geometry(intersection):
                    self.logger.error(
                        "Intersection operation produced invalid geometry"
                    )
                    return -1.0
            ...
            try:
                ...
                if intersection_area < 0 or union_area <= 0:
                    self.logger.error(
                        f"Invalid areas: intersection={intersection_area}, union={union_area}"
                    )
                    return -1.0

                if intersection_area > union_area:
                    self.logger.error(
                        f"Invalid geometry topology: intersection area ({intersection_area}) > union area ({union_area})"
                    )
                    return -1.0
    ...
    def _validate_geometries_for_iou(self, pred: Any, truth: Any) -> Optional[float]:
        ...
        if pred.geom_type not in valid_types:
            self.logger.error(
                f"Predicted geometry type '{pred.geom_type}' not suitable for IOU calculation"
            )
            return -1.0
        if truth.geom_type not in valid_types:
            self.logger.error(
                f"Truth geometry type '{truth.geom_type}' not suitable for IOU calculation"
            )
            return -1.0
        ...
        if pred.area < min_area:
            self.logger.warning(
                f"Predicted geometry area ({pred.area}) extremely small - potential precision issue"
            )
        if truth.area < min_area:
            self.logger.warning(
                f"Truth geometry area ({truth.area}) extremely small - potential precision issue"
            )
    ...
    def _validate_repaired_geometry(self, geom: Any, geom_type: str) -> bool:
        ...
        if geom is None:
            self.logger.error(
                f"Geometry repair failed: {geom_type} geometry is None after make_valid()"
            )
            return False

        if geom.is_empty:
            self.logger.error(
                f"Geometry repair failed: {geom_type} geometry is empty after make_valid()"
            )
            return False
        ...
        if geom.geom_type not in valid_types:
            self.logger.error(
                f"Geometry repair produced invalid type: {geom_type} geometry is now {geom.geom_type}"
            )
            return False
        ...
        if not geom.is_valid:
            self.logger.error(
                f"Geometry repair failed: {geom_type} geometry still invalid after make_valid()"
            )
            return False
    ...
    def _is_valid_area_geometry(self, geom: Any) -> bool:
        ...
        if geom.geom_type not in valid_types:
            self.logger.error(
                f"Geometry type '{geom.geom_type}' not suitable for area calculation"
            )
            return False
    ...
    def _assess_performance(
        self, iou: Optional[float], centroid_offset: Optional[float], terrain_class: str
    ) -> Dict[str, Any]:
        """
        ...
        centroid_target = centroid_thresholds.get(
            terrain_class, centroid_thresholds["default"]
        )
    ...
    def generate_reports(self) -> List[str]:
        ...
        self.logger.info(
            f"Report generation complete: {len(report_files)} files created"
        )
        return report_files

    def _write_performance_summary(
        self, results_df: pd.DataFrame, summary_path: Path
    ) -> None:
        ...
        with open(summary_path, "w") as f:
            ...
            if invalid_count > 0:
                f.write(
                    f"  Invalid geometry calculations: {invalid_count} ({invalid_count/len(results_df)*100:.1f}%)\n"
                )

            f.write(f"  Mean runtime: {results_df['Runtime_s'].mean():.1f}s\n")
            f.write(f"  Median runtime: {results_df['Runtime_s'].median():.1f}s\n")
            f.write(
                f"  95th percentile runtime: {results_df['Runtime_s'].quantile(0.95):.1f}s\n\n"
            )
            ...
            for terrain in ["flat", "moderate", "steep"]:
                ...
                if len(terrain_data) > 0:
                    ...
                    if len(valid_terrain_iou) > 0:
                        f.write(
                            f"    Mean IOU: {valid_terrain_iou.mean():.3f} (target: {target_iou:.3f})\n"
                        )
                    else:
                        f.write(
                            f"    Mean IOU: N/A - no valid calculations (target: {target_iou:.3f})\n"
                        )
            ...
            runtime_target_met = (
                results_df["Runtime_s"] <= runtime_target
            ).mean() * 100
            f.write(
                f"  • {runtime_target_met:.1f}% of basins completed within {runtime_target}s target\n"
            )
            ...
            if len(valid_for_target) > 0:
                ...
                f.write(
                    f"  • {target_met:.1f}% of valid calculations achieved ≥{iou_target:.0%} IOU\n"
                )
            else:
                f.write(
                    f"  • No valid IOU calculations to assess {iou_target:.0%} target\n"
                )

            # Identify problem cases (exclude None values)
            low_iou = results_df[
                (results_df["IOU"].notna()) & (results_df["IOU"] < 0.8)
            ]
            if len(low_iou) > 0:
                f.write(f"  • {len(low_iou)} basins with IOU < 0.8 (investigate)\n")

            # Report invalid geometry cases
            if "Geometry_Status" in results_df.columns:
                invalid_geom = results_df[
                    results_df["Geometry_Status"] == "invalid_geometry"
                ]
                if len(invalid_geom) > 0:
                    f.write(
                        f"  • {len(invalid_geom)} basins with invalid geometry calculations (critical issue)\n"
                    )

            slow_basins = results_df[results_df["Runtime_s"] > 60]
            if len(slow_basins) > 0:
                f.write(
                    f"  • {len(slow_basins)} basins took >60s (performance bottleneck)\n"
                )

    def run_complete_workflow(
        self, sample_path: str, truth_path: str
    ) -> Dict[str, Any]:
        """
        Run the complete benchmark workflow.
        ...
        try:
            ...
            return {"success": False, "error": str(e), "error_count": len(self.errors)}
...
def create_sample_config() -> str:
    ...
    sample_config = {
        ...
        "output_formats": {"json": True, "csv": True, "summary": True, "errors": True},
    }
...
Examples:
  ...
  python benchmark_runner.py --sample sample.csv --truth truth.gpkg --flowfinder-cmd /path/to/flowfinder
    ...
    parser.add_argument(
        "--sample", "-s", type=str, required=True, help="Path to basin sample CSV file"
    )
    ...
    try:
        ...
        if args.flowfinder_cmd:
            local_overrides = {"tool": {"executable": args.flowfinder_cmd}}
