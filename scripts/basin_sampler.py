...
class BasinSampler:
    ...
    def _load_config(
        ...
        default_config = {
            ...
            "export": {"csv": True, "gpkg": True, "summary": True, "error_log": True},
        }
    ...
    def _setup_logging(self) -> None:
        ...
        if not self.logger.handlers:
            ...
            self.logger.setLevel(
                logging.INFO
            )  # Default level, will be updated after config load
    ...
    def _is_large_dem_file(self, dem_path: Path) -> bool:
        ...
        if is_large:
            self.logger.info(
                f"DEM file is large ({file_size_mb:.1f} MB > {threshold_mb} MB), "
    ...
    def _validate_raster_integrity(
        self, dem: rasterio.DatasetReader, results: Dict[str, Any]
    ) -> None:
        ...
        try:
            if dem.crs is None:
                results["errors"].append(
                    "DEM has no coordinate reference system defined"
                )
                results["is_valid"] = False
                return
            ...
            if "EPSG:4326" in crs_str:
                results["warnings"].append(
                    "DEM uses geographic coordinates (EPSG:4326) - "
            ...
            if dem.bounds:
                ...
                if "EPSG:4326" in crs_str:
                    ...
                    if (
                        ...
                        results["warnings"].append(
                            "DEM extent appears outside Mountain West region"
                        )

        except Exception as e:
            results["warnings"].append(f"CRS validation warning: {e}")

    def _validate_dem_resolution(
        self, dem: rasterio.DatasetReader, results: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            if "EPSG:4326" not in str(dem.crs):  # Only check for projected coordinates
                if (
                    abs(pixel_size_x - expected_resolution) > expected_resolution * 0.1
                ):  # 10% tolerance
                    results["warnings"].append(
                        f"Unexpected resolution: {pixel_size_x:.3f}m "
            ...
            if "EPSG:4326" not in str(dem.crs):
                if pixel_size_x > 100:
                    results["warnings"].append(
                        f"Very coarse resolution: {pixel_size_x:.1f}m"
                    )
                elif pixel_size_x < 1:
                    results["warnings"].append(
                        f"Very fine resolution: {pixel_size_x:.3f}m"
                    )
            ...
            results["metrics"].update(
                {
                    ...
                    "is_square_pixels": abs(pixel_size_x - pixel_size_y)
                    < pixel_size_x * 0.01,
                }
            )
    ...
    def _validate_nodata_extent(
        ...
        try:
            ...
            sample_window = rasterio.windows.Window(
                0, 0, min(1000, dem.width), min(1000, dem.height)
            )
            ...
            if dem.nodata is not None:
                ...
                if nodata_percent > 80:
                    results["errors"].append(
                        f"Excessive NoData values: {nodata_percent:.1f}% of sample"
                    )
                    results["is_valid"] = False
                elif nodata_percent > 50:
                    results["warnings"].append(
                        f"High NoData percentage: {nodata_percent:.1f}% of sample"
                    )
                elif nodata_percent > 20:
                    results["warnings"].append(
                        f"Moderate NoData percentage: {nodata_percent:.1f}% of sample"
                    )

                # Check for reasonable NoData value
                if abs(dem.nodata) < 1e6:  # Reasonable range check
                    data_min = (
                        np.min(sample_data[~nodata_mask])
                        if not np.all(nodata_mask)
                        else np.nan
                    )
                    data_max = (
                        np.max(sample_data[~nodata_mask])
                        if not np.all(nodata_mask)
                        else np.nan
                    )
                    ...
                    if not np.isnan(data_min) and not np.isnan(data_max):
                        if data_min <= dem.nodata <= data_max:
                            results["warnings"].append(
                                "NoData value within data range - may cause confusion"
                            )
            else:
                results["metrics"].update({"nodata_value": None, "has_nodata": False})
                results["warnings"].append("No NoData value defined")

        except Exception as e:
            results["warnings"].append(f"NoData validation warning: {e}")

    def _validate_elevation_values(
        self, dem: rasterio.DatasetReader, results: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            sample_window = rasterio.windows.Window(
                0, 0, min(2000, dem.width), min(2000, dem.height)
            )
            sample_data = dem.read(1, window=sample_window)
            ...
            if elev_min > 4000:
                results["warnings"].append(
                    f"All elevations very high: min={elev_min:.1f}m"
                )

            # Check for flat areas (no elevation variation)
            if elev_std < 1.0:
                results["warnings"].append(
                    f"Very low elevation variation: std={elev_std:.3f}m"
                )

            # Check for unrealistic elevation ranges
            if (elev_max - elev_min) > 6000:
                results["warnings"].append(
                    f"Very large elevation range: {elev_max - elev_min:.1f}m"
                )

        except Exception as e:
            results["warnings"].append(f"Elevation validation warning: {e}")

    def _validate_spatial_coverage(
        self, dem: rasterio.DatasetReader, results: Dict[str, Any]
    ) -> None:
    ...
    def _validate_data_precision(
        ...
        try:
            ...
            if dtype in ["uint8", "int8"]:
                results["warnings"].append(
                    "8-bit data type may have insufficient precision for elevations"
                )
            elif dtype in ["float64"]:
                results["recommendations"].append(
                    "Consider float32 for better memory efficiency"
                )
            elif dtype in ["uint16", "int16"]:
                results["recommendations"].append(
                    "16-bit integer may be sufficient for most applications"
                )
            ...
            if "uint" in str(dtype):
                ...
                sample_window = rasterio.windows.Window(
                    0, 0, min(500, dem.width), min(500, dem.height)
                )
                sample_data = dem.read(1, window=sample_window)

                # For unsigned types, check if we're near the maximum value (indicating clipping)
                max_val = (
                    np.iinfo(dtype).max if "int" in str(dtype) else np.finfo(dtype).max
                )
                if np.any(sample_data == max_val):
                    results["warnings"].append(
                        "Data values at maximum for data type - possible clipping"
                    )
    ...
    def _log_validation_summary(self, results: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"Overall Status: {'VALID' if results['is_valid'] else 'INVALID'}"
        )
    ...
    def _is_dem_suitable_for_processing(self) -> bool:
        ...
        if nodata_percent > 80:
            self.logger.error(
                f"DEM has too much NoData ({nodata_percent:.1f}%) for reliable processing"
            )
            return False
        ...
        if elev_std < 0.1:
            self.logger.warning(
                f"DEM appears extremely flat (std={elev_std:.3f}m) - results may be unreliable"
            )

        return True

    def export_dem_validation_report(
        self, output_file: str = "dem_validation_report.json"
    ) -> None:
        """
            ...
            validation_results["metrics"].update(
                {
                    ...
                    "geometry_type": (
                        str(gdf.geom_type.iloc[0]) if len(gdf) > 0 else "unknown"
                    ),
    ...
    def _validate_required_columns(
        ...
        if empty_cols:
            results["warnings"].append(
                f"Found {len(empty_cols)} columns with empty names"
            )
    ...
    def _validate_column_data_types(
        ...
        if object_cols:
            ...
            if len(object_cols) > len(gdf.columns) / 2:
                results["warnings"].append(
                    "More than half of columns are object type - consider data type optimization"
                )
    ...
    def _is_compatible_dtype(self, actual_dtype: str, expected_type: str) -> bool:
        ...
        compatible_types = compatibility_map.get(
            expected_type_lower, [expected_type_lower]
        )

        return any(
            compatible_type in actual_dtype.lower()
            for compatible_type in compatible_types
        )

    def _validate_column_value_ranges(
        self,
        gdf: gpd.GeoDataFrame,
        schema_config: Dict[str, Any],
        results: Dict[str, Any],
        ...
        for col_name, range_config in value_ranges.items():
            ...
            if (
                not pd.api.types.is_numeric_dtype(col_data)
                and "string_length" not in range_config
            ):
                continue
            ...
            if "min" in range_config and pd.api.types.is_numeric_dtype(col_data):
                ...
                if min_val < range_config["min"]:
                    range_issues.append(
                        f"{col_name}: minimum value {min_val} below expected {range_config['min']}"
                    )
            ...
            if "max" in range_config and pd.api.types.is_numeric_dtype(col_data):
                ...
                if max_val > range_config["max"]:
                    range_issues.append(
                        f"{col_name}: maximum value {max_val} above expected {range_config['max']}"
                    )
            ...
            if "string_length" in range_config and col_data.dtype == "object":
                ...
                if max_length > expected_length:
                    range_issues.append(
                        f"{col_name}: maximum string length {max_length} exceeds expected {expected_length}"
                    )
    ...
    def _validate_null_values(
        ...
        for col_name in not_null_columns:
            ...
            if null_count > 0:
                ...
                null_issues.append(
                    f"{col_name}: {null_count} null values ({null_percentage:.1f}%)"
                )
        ...
        if high_null_columns:
            results["warnings"].append(
                f"Columns with high null percentages: {high_null_columns}"
            )

    def _validate_shapefile_geometry(
        self,
        gdf: gpd.GeoDataFrame,
        schema_config: Dict[str, Any],
        results: Dict[str, Any],
        ...
        if geometry_config.get("enable_diagnostics", True):
            ...
            geometry_stats = self.geometry_diagnostics.analyze_geometry_issues(
                gdf, "shapefile validation"
            )

            # Log detailed diagnostics
            if geometry_stats["total_invalid"] > 0:
                invalid_percentage = (
                    geometry_stats["total_invalid"] / geometry_stats["total_features"]
                ) * 100
                ...
                if geometry_stats["issue_types"]:
                    issue_summary = ", ".join(
                        [
                            f"{issue}: {count}"
                            for issue, count in geometry_stats["issue_types"].items()
                        ]
                    )
                ...
                if invalid_percentage > 10:  # More than 10% invalid
                    results["errors"].append(
                        f"High percentage of invalid geometries: {geometry_stats['total_invalid']} ({invalid_percentage:.1f}%)"
                    )
                    results["is_valid"] = False
                else:
                    results["warnings"].append(
                        f"Some invalid geometries found: {geometry_stats['total_invalid']} ({invalid_percentage:.1f}%)"
                    )
        else:
            ...
            if invalid_count > 0:
                ...
                if invalid_percentage > 10:  # More than 10% invalid
                    results["errors"].append(
                        f"High percentage of invalid geometries: {invalid_count} ({invalid_percentage:.1f}%)"
                    )
                    results["is_valid"] = False
                else:
                    results["warnings"].append(
                        f"Some invalid geometries found: {invalid_count} ({invalid_percentage:.1f}%)"
                    )
        ...
        if expected_geom_type:
            ...
            if len(actual_geom_types) > 1:
                results["warnings"].append(
                    f"Mixed geometry types found: {list(actual_geom_types)}"
                )
            elif actual_geom_types[0] != expected_geom_type:
                results["warnings"].append(
                    f"Unexpected geometry type: {actual_geom_types[0]}, expected {expected_geom_type}"
                )
        ...
        if empty_geoms > 0:
            ...
            if empty_percentage > 5:  # More than 5% empty
                results["errors"].append(
                    f"High percentage of empty geometries: {empty_geoms} ({empty_percentage:.1f}%)"
                )
                results["is_valid"] = False
            else:
                results["warnings"].append(
                    f"Some empty geometries found: {empty_geoms} ({empty_percentage:.1f}%)"
                )
                ...
                if rule_name == "unique_values":
                    ...
                    for col_name in rule_config.get("columns", []):
                        if col_name in gdf.columns:
                            ...
                            if duplicates > 0:
                                results["warnings"].append(
                                    f"{col_name}: {duplicates} duplicate values found"
                                )
                    ...
                    for col_name, expected_values in rule_config.items():
                        if col_name in gdf.columns:
                            unexpected_values = set(gdf[col_name].unique()) - set(
                                expected_values
                            )
                            if unexpected_values:
                                results["warnings"].append(
                                    f"{col_name}: unexpected values {list(unexpected_values)}"
                                )

                elif rule_name == "area_calculation":
                    # Validate area calculations if present
                    if (
                        "area_column" in rule_config
                        and rule_config["area_column"] in gdf.columns
                    ):
                        ...
                        area_diff = (
                            abs(actual_areas - reported_areas) / actual_areas * 100
                        )
                        high_diff_count = (area_diff > 10).sum()

                        if high_diff_count > 0:
                            results["warnings"].append(
                                f"Area discrepancies in {high_diff_count} features (>10% difference)"
                            )

            except Exception as e:
                results["warnings"].append(
                    f"Consistency rule '{rule_name}' failed: {e}"
                )

    def _log_schema_validation_summary(
        self, results: Dict[str, Any], file_description: str
    ) -> None:
    ...
    def _extract_dem_data_for_basin(
        self, basin_geometry, basin_id: str = "unknown"
    ) -> Optional[np.ndarray]:
        """
        ...
        try:
            ...
            if (
                self.dem
                and getattr(self, "huc12", None) is not None
                and self.huc12.crs
                and self.dem.crs
                and str(self.huc12.crs) != str(self.dem.crs)
            ):
    ...
    def _extract_dem_data_direct(
        self, basin_geometry, basin_id: str
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            self.logger.warning(
                f"Direct DEM extraction failed for basin {basin_id}: {e}"
            )
            return None

    def _extract_dem_data_chunked(
        self, basin_geometry, basin_id: str
    ) -> Optional[np.ndarray]:
        """
        ...
        try:
            ...
            if (
                window_width * window_height * bytes_per_pixel
                > chunk_size_mb * 1024 * 1024
            ):
                self.logger.debug(
                    f"Using chunked processing for basin {basin_id} "
                    f"(window: {window_width}x{window_height})"
                )
                return self._process_dem_window_chunked(
                    window, basin_geometry, chunk_size_pixels, basin_id
                )
            else:
            ...
            self.logger.warning(
                f"Chunked DEM extraction failed for basin {basin_id}: {e}"
            )
            return None

    def _process_dem_window_direct(
        self, window: Window, basin_geometry, basin_id: str
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            self.logger.warning(
                f"Direct window processing failed for basin {basin_id}: {e}"
            )
            return None

    def _process_dem_window_chunked(
        self, window: Window, basin_geometry, chunk_size_pixels: int, basin_id: str
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for row_start in range(0, window_height, chunk_size_pixels):
                ...
                for col_start in range(0, window_width, chunk_size_pixels):
                    ...
                    # Process chunk
                    chunk_data = self._process_dem_window_direct(
                        chunk_window,
                        basin_geometry,
                    ...
                    if not self._check_memory_usage(
                        f"chunked processing for basin {basin_id}"
                    ):
                        self._force_garbage_collection()
            ...
            self.logger.warning(
                f"Chunked window processing failed for basin {basin_id}: {e}"
            )
            return None
    ...
    def _load_huc12_data(self) -> None:
        ...
        if geometry_config.get("enable_diagnostics", True) or geometry_config.get(
            ...
            self.huc12 = self.geometry_diagnostics.diagnose_and_repair_geometries(
                self.huc12, "HUC12 boundaries"
            )

        # Perform schema validation after geometry repair
        schema_config = self.config.get("shapefile_schemas", {}).get("huc12", {})
        if schema_config:
            validation_results = self._validate_shapefile_schema(
                self.huc12, schema_config, "HUC12 boundaries"
            )
            ...
            if not validation_results["is_valid"]:
                if self.config.get("dem_validation", {}).get(
                    "fail_on_validation_errors", False
                ):
                    raise ValueError("HUC12 shapefile failed schema validation")
                else:
                    self.logger.warning(
                        "HUC12 shapefile validation failed - proceeding with caution"
                    )

        self.huc12 = self._validate_crs_transformation(
            self.huc12, "HUC12 boundaries", self.config["target_crs"]
        )
    ...
    def _load_flowlines_data(self) -> None:
        ...
        if geometry_config.get("enable_diagnostics", True) or geometry_config.get(
            ...
            self.flowlines = self.geometry_diagnostics.diagnose_and_repair_geometries(
        ...
        if schema_config:
            validation_results = self._validate_shapefile_schema(
                self.flowlines, schema_config, "NHD+ flowlines"
            )
            ...
            if not validation_results["is_valid"]:
                if self.config.get("dem_validation", {}).get(
                    ...
                    self.logger.warning(
                        "Flowlines shapefile validation failed - proceeding with caution"
                    )

        self.flowlines = self._validate_crs_transformation(
            self.flowlines, "NHD+ flowlines", self.config["target_crs"]
        )
        ...
        try:
            ...
            self.logger.debug(
                f"Spatial index built for {len(self.flowlines)} flowlines"
            )
        except Exception as e:
    ...
    def _load_dem_data(self) -> None:
        ...
        if dem_path.exists():
            ...
            self.logger.info(
                f"DEM loaded: {dem_info['width']}x{dem_info['height']} pixels, "
            ...
            if not validation_results["is_valid"]:
                self.logger.error(
                    "DEM validation failed - some processing may be unreliable"
                )
                if len(validation_results["errors"]) > 3:  # Too many critical errors
                    self.logger.error(
                        "Too many critical DEM errors - consider using a different DEM"
                    )
                    self.dem.close()
                    self.dem = None
            ...
            if estimated_memory_mb > 500:
                self.logger.warning(
                    f"DEM may require significant memory if fully loaded: "
    ...
    def filter_mountain_west_basins(self) -> None:
        ...
        if self.test_mode:
            self.logger.info(
                f"Test mode: skipping area filtering - {len(self.huc12)} basins retained"
            )
        else:
            area_min, area_max = self.config["area_range"]
            self.logger.debug(f"Area range: {area_min} - {area_max} km²")
            self.logger.debug(
                f"Area values used for filtering: {self.huc12['area_km2'].tolist()}"
            )

            area_mask = (self.huc12["area_km2"] >= area_min) & (
        ...
        if self.test_mode:
            self.logger.info(
                f"Test mode: skipping geographic filtering - {len(self.huc12)} basins retained"
            )
        else:
            ...
            if bounds_cfg.get("check_geographic_bounds", True):
                ...
                try:
                    ...
                    self.logger.error(
                        f"Failed to project basins to EPSG:4326 for geographic filtering: {e}"
                    )
                    raise
                mask = (
                ...
                self.logger.info(
                    f"Geographic filtering in WGS84: {len(self.huc12)}/{count_before} basins retained"
                )
            else:
        ...
        self.logger.info(
            f"Mountain West filtering complete: {final_count}/{initial_count} basins retained"
        )
    ...
    def compute_pour_points(self) -> None:
        ...
        if self.flowlines is None:
            self.logger.warning(
                "Flowlines data not available - using basin centroids as pour points"
            )
            centroids = self.huc12.geometry.centroid
            self.huc12["pour_point"] = centroids
            self.logger.info(
                f"Pour point computation complete: {len(centroids)} centroid points computed"
            )
            return
        ...
        if empty_count > 0:
            self.logger.warning(
                f"{empty_count} basins have empty geometries, skipping pour points."
            )
        ...
        for idx in tqdm(range(len(self.huc12)), desc="Computing pour points"):
            ...
            try:
                ...
                self.logger.error(
                    f"Error snapping pour point for basin {basin_id}: {e}"
                )
                raise
        ...
        self.logger.info(
            f"Pour point computation complete: {valid_points}/{len(pour_points)} valid points"
        )
    ...
    def compute_terrain_roughness(self) -> None:
        ...
        if self.dem is None:
            self.logger.warning(
                "DEM not available - skipping terrain roughness calculation"
            )
        ...
        if not self._is_dem_suitable_for_processing():
            self.logger.error(
                "DEM failed quality validation - skipping terrain roughness calculation"
            )
            self.huc12["slope_std"] = np.nan
            return

        self.logger.info(
            "Computing terrain roughness from DEM with memory management..."
        )
        ...
        for idx in tqdm(range(len(self.huc12)), desc="Computing terrain roughness"):
            try:
                ...
                elevation_data = self._extract_dem_data_for_basin(
                    basin_row.geometry, basin_id
                )
                ...
                slope_std = self._calculate_terrain_roughness(
                    elevation_data[valid_mask]
                )
                ...
                if processed_count % gc_frequency == 0:
                    ...
                    if not self._check_memory_usage(
                        f"terrain roughness (processed {processed_count})"
                    ):
                        memory_warnings += 1
                        self._force_garbage_collection()
                ...
                self.logger.warning(
                    f"Failed to compute terrain roughness for basin {basin_id}: {e}"
                )
        ...
        self.logger.info(
            f"Terrain roughness computation complete: {valid_slopes}/{len(slope_stds)} valid values"
        )
        if memory_warnings > 0:
            self.logger.warning(
                f"Encountered {memory_warnings} memory warnings during processing"
            )
    ...
    def compute_stream_complexity(self) -> None:
        ...
        if self.flowlines is None:
            self.logger.warning(
                "Flowlines data not available - setting stream density to 0.0"
            )
            self.huc12["stream_density"] = 0.0
            self.logger.info(
                f"Stream complexity computation complete: {len(self.huc12)} basins processed with 0.0 density"
            )
            return
        ...
        if empty_count > 0:
            self.logger.warning(
                f"{empty_count} basins have empty geometries, setting stream density to 0.0."
            )
        ...
        for idx in tqdm(range(len(self.huc12)), desc="Computing stream complexity"):
            ...
            try:
                ...
                candidate_idx = list(
                    self.flowlines_sindex.intersection(basin_geom.bounds)
                )
                ...
                self.logger.error(
                    f"Error computing stream complexity for basin {basin_id}: {e}"
                )
                raise

        self.huc12["stream_density"] = stream_densities
        self.logger.info(
            f"Stream complexity computation complete: {len(stream_densities)} basins processed"
        )
    ...
    def classify_basins(self) -> None:
        ...
        self.huc12["size_class"] = pd.cut(
            ...
            bins=[
                0,
                size_thresholds["small"],
                size_thresholds["medium"],
        ...
        self.logger.debug(
            f"Slope std values: {self.huc12['slope_std'].tolist() if 'slope_std' in self.huc12.columns else 'No slope_std column'}"
        )

        # Handle case where slope_std might not exist or be all NaN
        if (
            "slope_std" not in self.huc12.columns
            or self.huc12["slope_std"].isna().all()
        ):
            self.logger.warning(
                "No valid slope_std data - assigning all basins to 'flat' terrain class"
            )
            ...
            self.huc12["terrain_class"] = pd.cut(
                ...
                bins=[
                    0,
                    terrain_thresholds["flat"],
                    terrain_thresholds["moderate"],
                    float("inf"),
                ],
        ...
        self.logger.debug(
            f"Stream density values: {self.huc12['stream_density'].tolist()}"
        )

        # Handle case where all stream densities are the same (e.g., all 0.0)
        if self.huc12["stream_density"].nunique() <= 1:
            self.logger.warning(
                "All stream densities are identical - assigning all basins to 'low' complexity"
            )
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to create complexity quartiles: {e} - assigning all to 'low'"
                )
                self.huc12["complexity_score"] = 1

        self.logger.info("Basin classification complete")
        self.logger.info(
            f"Size distribution: {self.huc12['size_class'].value_counts().to_dict()}"
        )
        self.logger.info(
            f"Terrain distribution: {self.huc12['terrain_class'].value_counts().to_dict()}"
        )
        self.logger.info(
            f"Complexity distribution: {self.huc12['complexity_score'].value_counts().to_dict()}"
        )
    ...
    def stratified_sample(self) -> Dict[str, Any]:
        ...
        self.huc12["stratum"] = (
            self.huc12["size_class"].astype(str)
            + "_"
            + self.huc12["terrain_class"].astype(str)
            + "_"
        ...
        for stratum in self.huc12["stratum"].unique():
            ...
            if n_available < n_requested:
                self.logger.warning(
                    f"Stratum {stratum}: {n_available} available, {n_requested} requested"
                )
            ...
            if n_sample > 0:
                sampled = stratum_basins.sample(
                    n=n_sample, random_state=self.config["random_seed"]
                )
        ...
        if "pour_point" in self.sample.columns:
            self.sample["Pour_Point_Lon"] = self.sample["pour_point"].apply(
                lambda pt: pt.x if pt else None
            )
            self.sample["Pour_Point_Lat"] = self.sample["pour_point"].apply(
                lambda pt: pt.y if pt else None
            )
        ...
        self.logger.info(
            f"Stratified sampling complete: {len(self.sample)} basins sampled from {len(stratum_stats)} strata"
        )
        return summary
        ...
        if export_config.get("gpkg", True):
            ...
            try:
                ...
                sample_gdf = gpd.GeoDataFrame(
                    export_gdf_data, crs=self.config["target_crs"]
                )
                sample_gdf = self._validate_crs_transformation(
                    sample_gdf, "basin sample export", self.config["output_crs"]
                )
                ...
                self.logger.warning(
                    f"GeoPackage export failed: {e}. Skipping GPKG export."
                )
    ...
    def run_complete_workflow(
        self, output_prefix: str = "basin_sample"
    ) -> Dict[str, Any]:
        """
