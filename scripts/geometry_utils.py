...
class GeometryDiagnostics:
    ...
    def diagnose_and_repair_geometries(self, gdf: gpd.GeoDataFrame, data_description: str) -> gpd.GeoDataFrame:
        """
        Comprehensive geometry diagnostics and repair for geospatial data.
        ...
        """
        self.logger.info(f"Performing geometry diagnostics and repair for {data_description}")
        ...
        if geometry_stats['total_invalid'] > 0:
            self.logger.info(f"Attempting to repair {geometry_stats['total_invalid']} invalid geometries")
            gdf_repaired = self._apply_geometry_repairs(gdf_repaired, geometry_stats, data_description)
        
        # Final validation
        final_stats = self._analyze_geometry_issues(gdf_repaired, f"{data_description} (after repair)")
        ...
        if final_stats['total_invalid'] > 0:
            gdf_repaired = self._handle_remaining_invalid_geometries(gdf_repaired, final_stats, data_description)
        
        final_count = len(gdf_repaired)
        if final_count != original_count:
            self.logger.warning(f"Geometry repair resulted in {original_count - final_count} removed features")
        
        return gdf_repaired
    
    def analyze_geometry_issues(self, gdf: gpd.GeoDataFrame, data_description: str) -> Dict[str, Any]:
        """
    ...
    def _analyze_geometry_issues(self, gdf: gpd.GeoDataFrame, data_description: str) -> Dict[str, Any]:
        """
        Analyze and classify geometry issues with detailed diagnostics.
        ...
        for idx, geom in enumerate(gdf.geometry):
            ...
            if repair_strategy != 'none':
                stats['repair_candidates'][repair_strategy] = stats['repair_candidates'].get(repair_strategy, 0) + 1
            ...
            if not geom_analysis['is_valid'] and len(stats['detailed_diagnostics']) < 10:
    ...
    def _diagnose_single_geometry(self, geom, index: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not diagnosis['is_valid']:
                ...
                if 'self-intersection' in explanation_lower or 'self intersection' in explanation_lower:
                ...
                if 'duplicate point' in explanation_lower or 'repeated point' in explanation_lower:
                    diagnosis['issues'].append('duplicate_points')
                    diagnosis['repair_strategy'] = 'simplify'
                
                if 'orientation' in explanation_lower or 'clockwise' in explanation_lower:
                    diagnosis['issues'].append('wrong_orientation')
                    diagnosis['repair_strategy'] = 'orient_fix'
                
                if 'coordinate' in explanation_lower and ('nan' in explanation_lower or 'infinite' in explanation_lower):
                    diagnosis['issues'].append('invalid_coordinates')
                    diagnosis['repair_strategy'] = 'remove'
                    diagnosis['is_critical'] = True
                
                if 'too few points' in explanation_lower or 'insufficient' in explanation_lower:
                ...
                if 'nested' in explanation_lower or 'interior ring' in explanation_lower:
            ...
            if diagnosis['is_valid']:
                # Check for very small geometries
                if diagnosis['area'] < 1e-10 and geom.geom_type in ['Polygon', 'MultiPolygon']:
                    diagnosis['issues'].append('extremely_small_area')
                    diagnosis['repair_strategy'] = 'remove'
                
                # Check for very thin geometries
    ...
    def _apply_geometry_repairs(self, gdf: gpd.GeoDataFrame, geometry_stats: Dict[str, Any], 
                              data_description: str) -> gpd.GeoDataFrame:
        """
        ...
        for strategy, count in geometry_stats['repair_candidates'].items():
            ...
            for idx, geom in enumerate(gdf_repaired.geometry):
                ...
                try:
                    ...
                    if repaired_geom is not None and repaired_geom.is_valid:
                        gdf_repaired.iloc[idx, gdf_repaired.columns.get_loc('geometry')] = repaired_geom
                    ...
                    self.logger.warning(f"Repair strategy {strategy} failed for geometry {idx}: {e}")
                    failed_repairs.append({
                        'index': idx,
                        'strategy': strategy,
                        'error': str(e)
            ...
            self.logger.info(f"Strategy {strategy}: {strategy_successes}/{count} successful repairs")
    ...
    def _apply_repair_strategy(self, geom, strategy: str, index: int):
        ...
        try:
            ...
            self.logger.warning(f"Repair strategy {strategy} failed for geometry {index}: {e}")
            return None
    
    def _handle_remaining_invalid_geometries(self, gdf: gpd.GeoDataFrame, final_stats: Dict[str, Any], 
                                           data_description: str) -> gpd.GeoDataFrame:
        """
        ...
        """
        self.logger.warning(f"{final_stats['total_invalid']} geometries remain invalid after repair attempts")
        
        # Log details about remaining invalid geometries
        for diagnostic in final_stats['detailed_diagnostics']:
            self.logger.error(f"Index {diagnostic['index']}: {diagnostic['explanation']}")
        
        # Configuration-based handling
        config = self.config.get('geometry_repair', {})
        action = config.get('invalid_geometry_action', 'remove')  # 'remove', 'keep', 'convert_to_point'
        ...
        if action == 'remove':
            ...
            if removed_count > 0:
                self.logger.warning(f"Removing {removed_count} invalid geometries from {data_description}")
            ...
            if invalid_count > 0:
                self.logger.warning(f"Converting {invalid_count} invalid geometries to points")
            ...
            self.logger.warning(f"Keeping {final_stats['total_invalid']} invalid geometries as requested")
        
        return gdf
    
    def _log_geometry_repair_summary(self, original_stats: Dict[str, Any], final_stats: Dict[str, Any], 
        ...
        if hasattr(self, 'repair_stats') and self.repair_stats:
            ...
            if repair_counts:
                ...
                for strategy, stats in repair_counts.items():
                    ...
                    self.logger.info(f"  - {strategy}: {stats['successful']}/{stats['attempted']} ({success_rate:.1f}%)")
        
        # Critical errors
        if original_stats['critical_errors']:
            self.logger.error(f"Found {len(original_stats['critical_errors'])} critical geometry errors:")
            for error in original_stats['critical_errors'][:5]:  # Show first 5
                self.logger.error(f"  - Index {error['index']}: {error['explanation']}")
        
        # Recommendations
        recommendations = self._generate_geometry_recommendations(original_stats, final_stats)
        if recommendations:
            self.logger.info("Recommendations:")
            for rec in recommendations:
                self.logger.info(f"  - {rec}")
    
    def _generate_geometry_recommendations(self, original_stats: Dict[str, Any], 
                                         final_stats: Dict[str, Any]) -> List[str]:
        """
        ...
        if original_stats['total_features'] > 0:
            invalid_rate = (original_stats['total_invalid'] / original_stats['total_features']) * 100
            if invalid_rate > 20:
                recommendations.append(f"High invalid geometry rate ({invalid_rate:.1f}%) - consider data source quality")
        
        # Specific issue patterns
        issue_types = original_stats.get('issue_types', {})
        
        if issue_types.get('self_intersection', 0) > 5:
            recommendations.append("Many self-intersection errors - consider simplifying input data")
        
        if issue_types.get('duplicate_points', 0) > 10:
            recommendations.append("Many duplicate point errors - consider coordinate precision settings")
        
        if issue_types.get('extremely_small_area', 0) > 0:
            recommendations.append("Very small geometries detected - consider minimum area thresholds")
        
        # Repair effectiveness
        if hasattr(self, 'repair_stats') and self.repair_stats:
            repair_counts = self.repair_stats['repair_counts']
            total_attempted = sum(stats['attempted'] for stats in repair_counts.values())
            total_successful = sum(stats['successful'] for stats in repair_counts.values())
            
            if total_attempted > 0:
                overall_success_rate = (total_successful / total_attempted) * 100
                if overall_success_rate < 50:
                    recommendations.append("Low repair success rate - consider alternative data sources")
