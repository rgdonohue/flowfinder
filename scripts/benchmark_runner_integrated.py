...
class MultiBenchmarkRunner:
    ...
    def __init__(
        ...
        for tool in self.tools:
            try:
                self.tool_configs[tool] = self.config_manager.get_tool_config(
                    tool, local_overrides
                )
                self.tool_adapters[tool] = self.config_manager.get_tool_adapter(
                    tool, self.tool_configs[tool]
                )
        ...
        self.logger.info(
            f"MultiBenchmarkRunner initialized with {len(self.tool_adapters)} tools"
        )
    ...
    def run_multi_tool_comparison(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        successful_tools = [
            tool for tool, result in results.items() if result["success"]
        ]
        failed_tools = [
        ...
        if successful_tools:
            ...
            fastest_tool = min(
                successful_tools, key=lambda t: results[t]["runtime_seconds"]
            )
        else:
            avg_runtime = None
            fastest_tool = None
        ...
        if output_format == "json":
            ...
            with open(output_file, "w") as f:
                ...
                if "summary" in results:
                    ...
                    f.write(
                        f"Successful Tools: {', '.join(summary['successful_tool_names'])}\n"
                    )
                    f.write(
                        f"Failed Tools: {', '.join(summary['failed_tool_names'])}\n"
                    )
  ...
  python benchmark_runner_integrated.py --env testing --tools flowfinder taudem --test
    ...
    try:
        ...
        runner = MultiBenchmarkRunner(
            environment=args.environment, tools=tools, output_dir=args.output
        )
