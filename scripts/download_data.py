...
class DataDownloader:
    ...
    def download_file(self, url: str, output_path: Path, description: str) -> bool:
        ...
        try:
            ...
            response = requests.get(
                url, stream=True, timeout=self.config["download"]["timeout_seconds"]
            )
    ...
    def download_huc12(self) -> bool:
        ...
        output_path = (
            Path(self.config["storage"]["raw_data_dir"])
            / "huc12"
    ...
    def download_nhd_hr(self) -> bool:
        ...
        def download_huc4_region(huc4):
            ...
            output_path = (
                Path(self.config["storage"]["raw_data_dir"])
                / "nhd_hr"
                / f"NHDPlus_H_{huc4}_HU4_GDB.zip"
            )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_huc4 = {
                executor.submit(download_huc4_region, huc4): huc4
                for huc4 in huc4_regions
            }
    ...
    def download_dem(self) -> bool:
        ...
        def download_dem_tile(tile_path):
            url = (
                f"{base_url}{tile_path}/USGS_{tile_path.replace('/', '_')}_20221115.tif"
            )
            output_path = (
                Path(self.config["storage"]["raw_data_dir"])
                / "dem"
                / f"USGS_{tile_path.replace('/', '_')}_20221115.tif"
            )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_tile = {
                executor.submit(download_dem_tile, tile): tile
    ...
    def validate_downloads(self) -> Dict[str, bool]:
        ...
        validation_results["huc12"] = huc12_path.exists() and any(
            huc12_path.glob("*.shp")
        )
    ...
    def generate_metadata(self):
        ...
        # Save metadata
        metadata_path = (
            Path(self.config["storage"]["metadata_dir"]) / "download_metadata.json"
        )
        import json
...
Examples:
  ...
  python download_data.py --config custom_data_sources.yaml --all
    ...
    parser.add_argument(
        "--all", action="store_true", help="Download all required datasets"
    )
    parser.add_argument(
        "--dataset",
        choices=["huc12", "nhd_hr", "dem"],
