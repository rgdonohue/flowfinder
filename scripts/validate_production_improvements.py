...
def validate_optimized_algorithms():
    ...
    if opt_file.exists():
        ...
        checks.append(
            (
                "OptimizedDepressionFilling",
                "class OptimizedDepressionFilling" in content,
            )
        )
        checks.append(
            ("Priority flood algorithm", "priority-flood algorithm" in content)
        )
        checks.append(
            ("OptimizedFlowAccumulation", "class OptimizedFlowAccumulation" in content)
        )
        checks.append(("Topological sorting", "topological sorting" in content))
        checks.append(
            ("OptimizedPolygonCreation", "class OptimizedPolygonCreation" in content)
        )
        checks.append(
            ("Morphological operations", "morphological operations" in content)
        )
...
def validate_advanced_algorithms():
    ...
    if adv_file.exists():
        ...
        checks.append(
            ("DInfinityFlowDirection", "class DInfinityFlowDirection" in content)
        )
        ...
        checks.append(
            ("HydrologicEnforcement", "class HydrologicEnforcement" in content)
        )
        checks.append(
            ("Flow direction validation", "_is_flow_direction_valid" in content)
        )
    else:
...
def validate_core_integration():
    ...
    if core_file.exists():
        ...
        checks.append(
            ("CRSHandler import", "from .crs_handler import CRSHandler" in content)
        )
        checks.append(
            (
                "Optimized algorithms import",
                "from .optimized_algorithms import" in content,
            )
        )
        checks.append(
            (
                "Scientific validation import",
                "from .scientific_validation import" in content,
            )
        )
        ...
        checks.append(
            ("Tuple return type", "Tuple[Polygon, Dict[str, Any]]" in content)
        )
    else:
...
def validate_flow_direction_updates():
    ...
    if flow_file.exists():
        ...
        checks.append(
            (
                "Real D-infinity implementation",
                "calculate_dinf_flow_direction" in content,
            )
        )
...
def main():
    ...
    if all_passed:
        ...
        missing_categories = [
            cat for cat, passed in zip(categories, category_results) if not passed
        ]
