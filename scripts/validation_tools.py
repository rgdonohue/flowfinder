...
def check_shapefile_quality(shapefile_path: str) -> Dict[str, Any]:
    ...
    try:
        if GEOPANDAS_AVAILABLE:
            ...
            if len(invalid) > 0:
                result['issues'].append(f"{len(invalid)} invalid geometries: " + ", ".join([explain_validity(geom) for geom in invalid.geometry[:5]]))
            # Attribute missingness
            for col in gdf.columns:
                if gdf[col].isnull().any():
                    result['missing_attributes'].append(col)
            if result['missing_attributes']:
                result['issues'].append(f"Missing values in: {result['missing_attributes']}")
        else:
            # Fallback: use Fiona
            with fiona.open(shapefile_path) as src:
                ...
                if invalid_count > 0:
                    result['issues'].append(f"{invalid_count} invalid geometries (Fiona)")
...
# ----------------------
def validate_intermediate_csv(csv_path: str, required_columns: List[str]) -> Dict[str, Any]:
    """
...
def validate_geopackage(gpkg_path: str, required_layers: Optional[List[str]] = None) -> Dict[str, Any]:
    """
...
# Performance Regression Detection
# ----------------------
def detect_performance_regression(current_metrics: Dict[str, Any], baseline_metrics_path: str, thresholds: Dict[str, float]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for metric, threshold in thresholds.items():
            if metric in current_metrics and metric in baseline:
                ...
                result['details'][metric] = {'baseline': base, 'current': curr, 'change': change}
...
Examples:
  ...
  python validation_tools.py --validate-config config/pipeline_config.yaml --schema config/pipeline_schema.json
    ...
    parser.add_argument('--check-shapefile', type=str, help='Path to shapefile or GeoPackage to check')
    parser.add_argument('--check-raster', type=str, help='Path to raster (DEM) to check')
    parser.add_argument('--validate-csv', type=str, help='Path to CSV to validate')
    parser.add_argument('--columns', type=str, help='Comma-separated list of required columns for CSV')
    parser.add_argument('--validate-gpkg', type=str, help='Path to GeoPackage to validate')
    parser.add_argument('--layers', type=str, help='Comma-separated list of required layers for GeoPackage')
    parser.add_argument('--verify-format', type=str, help='Path to output file to verify format')
    parser.add_argument('--format', type=str, help='Expected format: csv, json, geojson, gpkg')
    parser.add_argument('--detect-regression', action='store_true', help='Detect performance regression')
    parser.add_argument('--current-metrics', type=str, help='Path to current metrics JSON')
    parser.add_argument('--baseline', type=str, help='Path to baseline metrics JSON')
    parser.add_argument('--thresholds', type=str, help='Comma-separated metric=threshold pairs (e.g., iou=0.05,runtime=0.2)')
    parser.add_argument('--validate-config', type=str, help='Path to YAML config to validate')
    ...
    if args.check_shapefile:
        ...
        if not (args.current_metrics and args.baseline and args.thresholds):
            print("--current-metrics, --baseline, and --thresholds are required for regression detection")
            sys.exit(1)
        thresholds = {k: float(v) for k, v in (item.split('=') for item in args.thresholds.split(','))}
        with open(args.current_metrics) as f:
            current_metrics = json.load(f)
        result = detect_performance_regression(current_metrics, args.baseline, thresholds)
        print(json.dumps(result, indent=2))
